...
class TriageAgent:
    ...
    def classify_document(self, pdf_path: Path) -> DocumentProfile:
        ...
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            estimated_cost = self._estimate_extraction_cost(origin_type, layout_complexity)
    ...
    def _detect_language(pdf: pdfplumber.PDF) -> Tuple[str, float]:
        ...
        For now, we assume English with high confidence. This can be replaced
        with a real language detection library (e.g. `langdetect`) later.
        """
        return "en", 0.99
